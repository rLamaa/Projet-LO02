...
public class ControleurJest {
	...
	private void gererFaireOffre() {
		...
		if (cartes.size() < 2) {
			JOptionPane.showMessageDialog(vue.getFrame(), "Vous n'avez pas assez de cartes pour faire une offre.",
	...
	private void gererChoisirCarte() {
		...
		if (offresActuelles == null) {
			JOptionPane.showMessageDialog(vue.getFrame(), "Aucune offre disponible pour le moment.", "Information",
					JOptionPane.INFORMATION_MESSAGE);
		...
		if (offresDisponibles.isEmpty()) {
			JOptionPane.showMessageDialog(vue.getFrame(), "Aucune offre disponible.", "Information",
					JOptionPane.INFORMATION_MESSAGE);
		...
		for (int i = 0; i < offresDisponibles.size(); i++) {
			...
			if (offresVisibles) {
				options[i] = (i + 1) + ". [" + o.getProprietaire().getNom() + "] Carte 1: " + o.getCarteVisible()
						+ " | Carte 2: " + o.getCarteCachee();
			} else {
				options[i] = (i + 1) + ". [" + o.getProprietaire().getNom() + "] Visible: " + o.getCarteVisible()
						+ " | Cachée: [?]";
			}
		}
